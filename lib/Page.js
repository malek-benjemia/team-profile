...
class Page {
    ...
    generatePage(manager, engineers, interns) {
        ...
        let managerText = "<h2>The Manager</h2></br><div>The manager name: " +manager[0].name+ "</br>The manager ID: "+manager[0].ID+"</br>The manager email: "+manager[0].email+"</br>The manager office number: "+manager[0].officeNb+"</div></br>";
        let engineersText = "<h2>The Engineers</h2></br>";
        for (var i = 0; i < engineers.length; i++)  {
            engineersText=engineersText+ "<div>The engineer name: " +engineers[i].name+ "</br>The engineer ID: "+engineers[i].ID+"</br>The engineer email: "+engineers[i].email+"</br>The engineer GitHub: <a href='https://github.com/"+engineers[i].git+">GitHub</a></div></br>"
          };
        let internsText = "<h2>The Interns</h2></br>";
        for (var i = 0; i < interns.length; i++)  {
              internsText=internsText+ "<div>The intern name: " +interns[i].name+ "</br>The intern ID: "+interns[i].ID+"</br>The intern email: "+interns[i].email+"</br>The intern school: "+interns[i].school+"</div></br>"
            };

        console.log(`${topText}</br>${managerText}</br>${engineersText}</br>${internsText}`);
    }
